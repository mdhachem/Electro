...
class CartController extends AbstractController
{
     ...
     */
    function cart(ProduitRepository $repo)
    {
        $session = new Session();
     ...
     */
    function addToCart(Produit $produit, Request $request)
    {
        $session = new Session();
        @$cart = $session->get('session_cart');
        $quantite = 0;
     ...
     */
    function addOneToCart(Produit $prd)
    {
        $session = new Session();
        @$cart = $session->get('session_cart');
        @$cart[$prd->getId()]++;
     ...
     */
    function removeOneToCart(Produit $prd)
    {
        $session = new Session();
        @$cart = $session->get('session_cart');
        @$cart[$prd->getId()]--;
